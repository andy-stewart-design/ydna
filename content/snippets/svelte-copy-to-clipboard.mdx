...
I developed this feature while exploring how to build an interacive repository for an SVG icon set. I wanted a simple way to display a bunch of cards on a page that, when clicked, would copy the code for a specific icon to the user's clipboard.

For me, a custom action seemed to be the most effective of the various ways that you could implement this in Svelte. Actions are an elegant, if underused, way of encapsulating reusable logic in Svelte. They are essentially element-level lifecycle hooks, which allow you to easily attach a function to an element — or in my case, to a hundred elements — that will run when that element is added to the DOM.
...
- In general, if you use Typescript and you want to write a custom action in Svelte, your editor is going to yell at you when try to use it if you don't first extend Svelte's JSX typings. Create a file named `additional-svelte-jsx.d.ts` and follow the format described [in the docs](https://github.com/sveltejs/language-tools/blob/master/docs/preprocessors/typescript.md#im-using-an-attributeevent-on-a-dom-element-and-it-throws-a-type-error). You can see the implementation for the `useClipboard` action [here](https://github.com/andy-stewart-design/svelte-sandbox/blob/main/src/lib/types/additional-svelte-jsx.d.ts).
